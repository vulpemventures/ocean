...
func (w *Wallet) SignPset(args SignPsetArgs) (string, error) {
	...
	for i, in := range ptx.Inputs {
		...
		if ok {
			if err := w.signInput(ptx, i, path, args.sighashType()); err != nil {
				return "", err
			}
		}
	}
...
func (w *Wallet) signTxInput(
	...
	if len(redeemScript) <= 0 {
		unconf, _ := payment.FromPublicKeys(pubkeys, len(pubkeys), nil, nil)
		redeemScript = unconf.WitnessScript
	}
	...
	hashForSignature := unsignedTx.HashForWitnessV0(
		int(inIndex), redeemScript, value, sighashType,
	)
...
func (w *Wallet) signInput(
	...
	hashForSignature := unsingedTx.HashForWitnessV0(
		inIndex, input.WitnessScript, ptx.Inputs[inIndex].GetUtxo().Value, input.SigHashType,
	)
