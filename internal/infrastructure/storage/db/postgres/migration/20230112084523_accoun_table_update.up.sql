...
ALTER TABLE account ADD COLUMN label varchar(500) UNIQUE;

ALTER TABLE account_script_info ADD COLUMN fk_account_namespace varchar(500);

ALTER TABLE account_script_info ADD FOREIGN KEY (fk_account_namespace) REFERENCES account(namespace) ON DELETE CASCADE;
...
ALTER TABLE tx_input_account ADD FOREIGN KEY (fk_account_namespace) REFERENCES account(namespace) ON DELETE CASCADE;

ALTER TABLE utxo DROP COLUMN account_name;

ALTER TABLE utxo ADD COLUMN fk_account_namespace varchar(500);

ALTER TABLE utxo ADD FOREIGN KEY (fk_account_namespace) REFERENCES account(namespace) ON DELETE CASCADE;
