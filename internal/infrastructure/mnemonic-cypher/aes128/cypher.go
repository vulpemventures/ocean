...
func (c *Cypher) Encrypt(mnemonic, password []byte) ([]byte, error) {
	...
	ciphertext := gcm.Seal(nonce, nonce, mnemonic, nil)
...
func (c *Cypher) Decrypt(encryptedMnemonic, password []byte) ([]byte, error) {
	...
	salt := encryptedMnemonic[len(encryptedMnemonic)-32:]
	data := encryptedMnemonic[:len(encryptedMnemonic)-32]
	...
	if err != nil {
		return nil, fmt.Errorf("invalid passpharse")
	}
